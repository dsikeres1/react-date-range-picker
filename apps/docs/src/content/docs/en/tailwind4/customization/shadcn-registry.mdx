...
Full [shadcn/ui registry](https://ui.shadcn.com/docs/registry) integration is planned. This will allow installing the date picker via:

```bash
npx shadcn@latest add react-date-range-picker
```

Until then, install the npm package directly and use the Compound Component API for any structural customization.
