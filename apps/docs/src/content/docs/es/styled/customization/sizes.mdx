...
El paquete con estilo incluye 4 variantes de tamaño incorporadas que escalan la tipografía, el espaciado, las dimensiones de las celdas y las dimensiones del disparador proporcionalmente.

## Tamaños Disponibles

| Tamaño    | Tamaño de Celda | Fuente Base | Ancho Mínimo del Disparador |
| --------- | --------------- | ----------- | --------------------------- |
...
`medium` es el predeterminado.

## Uso

Pasa la prop `size` a cualquier componente del selector:
...
## Qué Cambia por Tamaño

Cada variante de tamaño sobrescribe un conjunto de variables CSS a través de una clase (p. ej., `rdrp-size-small`). Esto es lo que cada tamaño ajusta:
...
Crea tu propio tamaño definiendo una clase CSS que sobrescriba las variables de tamaño:
...
Luego aplícalo a través de un contenedor:
...
O con la API de Componentes Compuestos, añade la clase a Root:
...
También puedes sobrescribir las variables de tamaño mediante estilos en línea:
...
<Admonition type="tip">
  El tamaño `medium` es el predeterminado y no aplica ninguna sobreescritura. Las variables base en
  `variables.css` son los valores de `medium`.
</Admonition>
