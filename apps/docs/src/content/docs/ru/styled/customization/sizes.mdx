...
Пакет `styled` включает 4 встроенных варианта размера, которые пропорционально масштабируют типографику, отступы, размеры ячеек и размеры триггера.

## Доступные размеры

| Размер    | Размер ячейки | Базовый шрифт | Мин. ширина триггера |
| --------- | ------------- | ------------- | -------------------- |
...
`medium` — это значение по умолчанию.

## Использование

Передайте проп `size` в любой компонент выбора даты:
...
## Что меняется в зависимости от размера

Каждый вариант размера переопределяет набор CSS-переменных через класс (например, `.rdrp-size-small`). Вот что настраивает каждый размер:
...
Создайте свой собственный размер, определив CSS-класс, который переопределяет переменные размеров:
...
Затем примените его через обертку:
...
Или с помощью Compound Component API, добавьте класс в Root:
...
Вы также можете переопределить переменные размеров с помощью инлайн-стилей:
...
<Admonition type="tip">
  Размер `medium` является значением по умолчанию и не применяет никаких переопределений. Базовые
  переменные в `variables.css` являются значениями для размера medium.
</Admonition>
