...
Никакого дополнительного импорта CSS не требуется. Компоненты напрямую используют служебные классы Tailwind.
...
В этом случае импортируйте предоставленный `reset.css`, чтобы вручную инициализировать стили кнопок:
