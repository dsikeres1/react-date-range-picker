...
<Admonition type="info" title="Yakında Gelecek">
Tam [shadcn/ui kayıt defteri](https://ui.shadcn.com/docs/registry) entegrasyonu planlanmaktadır. Bu, tarih seçicinin şu şekilde kurulmasına olanak tanıyacaktır:

```bash
npx shadcn@latest add react-date-range-picker
```

O zamana kadar, npm paketini doğrudan kurun ve herhangi bir yapısal özelleştirme için Bileşik Bileşen API'sini kullanın.
