...
테마를 사용자 정의하는 가장 간단한 방법은 Compound Component API를 사용하는 것입니다. 각 파트는 `className` prop을 받으며, 이 prop은 `twMerge`를 사용하여 기본값과 병합되므로 충돌하는 유틸리티가 올바르게 해결됩니다.
...
<Admonition type="tip">
  구체적인 색상 클래스 대신 (shadcn/ui와 같은) 시맨틱 토큰 기반 테마를 원한다면, CSS 변수 기반
