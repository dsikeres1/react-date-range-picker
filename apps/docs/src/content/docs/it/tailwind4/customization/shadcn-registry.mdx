...
전체 [shadcn/ui 레지스트리](httpss://ui.shadcn.com/docs/registry) 통합이 계획되어 있습니다. 이를 통해 다음 명령어로 날짜 선택기를 설치할 수 있게 됩니다:

```bash
npx shadcn@latest add react-date-range-picker
```

그때까지는 npm 패키지를 직접 설치하고, 구조적 커스터마이징이 필요할 경우 Compound Component API를 사용하세요.
