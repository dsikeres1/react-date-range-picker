...
<Admonition type="info" title="Bientôt disponible">
L'intégration complète avec le [registre shadcn/ui](https://ui.shadcn.com/docs/registry) est prévue. Cela permettra d'installer le sélecteur de date via :

```bash
npx shadcn@latest add react-date-range-picker
```

D'ici là, installez le paquet npm directement et utilisez l'API de Composant Composé pour toute personnalisation structurelle.
