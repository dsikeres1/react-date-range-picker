...
Das Styled-Paket enthält 4 integrierte Größenvarianten, die Typografie, Abstände, Zellengrößen und Auslöserdimensionen proportional skalieren.

## Verfügbare Größen

| Größe     | Zellengröße | Basisschriftgröße | Min. Breite des Auslösers |
| --------- | ----------- | ----------------- | ------------------------- |
...
`medium` ist die Standardeinstellung.

## Verwendung

Übergeben Sie die `size`-Prop an eine beliebige Picker-Komponente:
...
## Was sich pro Größe ändert

Jede Größenvariante überschreibt einen Satz von CSS-Variablen über eine Klasse (z. B. `rdrp-size-small`). Hier ist, was jede Größe anpasst:
...
Erstellen Sie Ihre eigene Größe, indem Sie eine CSS-Klasse definieren, die die Größenvariablen überschreibt:
...
Wenden Sie sie dann über einen Wrapper an:
...
Oder fügen Sie mit der Compound Component API die Klasse zu Root hinzu:
...
Sie können Größenvariablen auch über Inline-Stile überschreiben:
...
<Admonition type="tip">
  Die Größe `medium` ist die Standardeinstellung und wendet keine Überschreibungen an. Die
  Basisvariablen in `variables.css` sind die `medium`-Werte.
</Admonition>
