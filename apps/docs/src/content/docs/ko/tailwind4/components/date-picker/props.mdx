...
| `initialMonth`        | `Date`                                        | --          | 초기 표시되는 달                      |
| `size`                | `"small" \| "medium" \| "large" \| "x-large"` | `"medium"`  | 컴포넌트 크기                         |
| `weekStartsOn`        | `WeekDay`                                     | `"sunday"`  | 주의 시작일                           |
| `isDateUnavailable`   | `(date: Date) => boolean`                     | --          | 특정 날짜를 비활성화 처리             |
| `displayFormat`       | `string`                                      | --          | 커스텀 표시 형식 (예: `"YYYY/MM/DD"`) |
...
| `numberOfMonths`      | `number`                                      | `1`         | 표시할 달의 수                        |
...
| `placeholder`         | `string`                                      | --          | 트리거의 placeholder 텍스트           |
