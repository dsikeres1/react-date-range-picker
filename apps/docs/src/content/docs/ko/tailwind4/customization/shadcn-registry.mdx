...
<Admonition type="info" title="출시 예정">
완벽한 [shadcn/ui 레지스트리](https://ui.shadcn.com/docs/registry) 연동이 계획되어 있습니다. 이를 통해 다음과 같이 데이트 피커를 설치할 수 있게 됩니다:

```bash
npx shadcn@latest add react-date-range-picker
```

그 전까지는 npm 패키지를 직접 설치하고 컴파운드 컴포넌트(Compound Component) API를 사용하여 구조를 커스터마이징하세요.
