...
| `onChange`               | `(value: { start: Date \| null; end: Date \| null }) => void` | --          | 값이 변경될 때의 콜백                                            |
| `time`                   | `TimeConfig`                                                  | --          | 시간 패널 설정 (시작과 끝 모두에 적용)                           |
| `presets`                | `DateRangePreset[]`                                           | --          | 미리 정의된 날짜 범위 프리셋                                     |
| `maxDays`                | `number`                                                      | --          | 범위 내 허용되는 최대 일수 (포함)                                |
| `minDays`                | `number`                                                      | --          | 범위 내 필요한 최소 일수 (포함)                                  |
| `allowSingleDateInRange` | `boolean`                                                     | `true`      | 시작과 끝이 같은 범위를 허용                                     |
...
| `name`                   | `string`                                                      | --          | 폼 제출을 위한 숨겨진 입력 필드 이름 (시작 날짜-시간)            |
| `endName`                | `string`                                                      | --          | 종료 날짜-시간을 위한 숨겨진 입력 필드 이름 (기본값: `name-end`) |
