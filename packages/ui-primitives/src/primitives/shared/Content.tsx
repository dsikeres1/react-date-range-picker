...
export const Content = forwardRef<HTMLDivElement, ContentProps>(
  (
    { className, popupClassName, inlineClassName, portalRoot, children, cn: cnFn },
    forwardedRef,
  ) => {
    ...
    const activeDescendant = focusedDate
      ...
      : undefined;
    const sizeClassName = size ? `rdrp-size-${size}` : undefined;

    if (inline) {
      return (
        <div
          ref={mergeRefs(forwardedRef, popupRef, localPopupRef)}
          className={cn("rdrp-root", className, inlineClassName, sizeClassName)}
    ...
    return (
      <FloatingPortal root={portalRoot !== undefined ? portalRoot : context.containerRef.current}>
        <FloatingFocusManager context={floatingContext} initialFocus={localPopupRef} returnFocus>
          <div
            ...
            className={cn("rdrp-root", popupClassName, className, sizeClassName)}
            onKeyDown={handleKeyDown}
            tabIndex={0}
            role="dialog"
            aria-label={locale.placeholder}
